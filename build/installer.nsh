...
Function QuchenRadioWriteInstallMarker
  ...
  ${If} ${Errors}
    MessageBox MB_ICONSTOP|MB_OK "无法写入安装目录安全标记，安装已停止。请选择可写入的 QuchenRadio 专用文件夹。"
    Abort
  ${EndIf}
  FileWrite $0 "QuchenRadio install root marker.$\r$\n"
  FileClose $0
FunctionEnd
!endif
...
Function un.QuchenRadioAbortUnsafeUninstallRoot
  ...
  ${If} $0 != "QuchenRadio"
  ${AndIf} $0 != "quchen"
    MessageBox MB_ICONSTOP|MB_OK "卸载已中止：$INSTDIR 不是 QuchenRadio 专用安装目录。为避免误删用户文件，请手动删除 QuchenRadio 程序文件。"
    Abort
  ${EndIf}
  IfFileExists "$INSTDIR\${QUCHEN_INSTALL_MARKER}" safe 0
  MessageBox MB_ICONSTOP|MB_OK "卸载已中止：$INSTDIR 不是 QuchenRadio 专用安装目录，缺少安全标记 ${QUCHEN_INSTALL_MARKER}。为避免误删用户文件，请手动删除 QuchenRadio 程序文件。"
  Abort
safe:
FunctionEnd
!endif
...
Function QuchenRadioWelcomeShow
  ...
  ${NSD_CreateLabel} 22u 42u 226u 30u "QuchenRadio 安装"
  Pop $0
  ...
  ${NSD_CreateLabel} 22u 96u 238u 24u "为这台电脑安装 QuchenRadio。默认安装到 C:\QuchenRadio；选择其它位置时会自动落入专用 QuchenRadio 子文件夹。"
  Pop $0
...
Function QuchenRadioDirectoryBrowse
  nsDialogs::SelectFolderDialog "选择 QuchenRadio 安装文件夹" "$INSTDIR"
  Pop $0
  ${If} $0 != error
  ${AndIf} $0 != ""
    Push "$0"
...
Function QuchenRadioDirectoryShow
  ...
  ${NSD_CreateLabel} 22u 40u 238u 24u "你可以使用默认路径，也可以选择其它磁盘或文件夹。安装器会自动创建专用 QuchenRadio 子目录，避免卸载时影响其它文件。"
  Pop $0
  SendMessage $0 ${WM_SETFONT} $QuchenRadioBodyFont 1
  SetCtlColors $0 "4B5263" "FFFFFF"
  ...
  ${NSD_CreateLabel} 22u 122u 238u 12u "默认推荐：C:\QuchenRadio；选择现有文件夹会自动追加 QuchenRadio。"
  Pop $0
  SendMessage $0 ${WM_SETFONT} $QuchenRadioSmallFont 1
