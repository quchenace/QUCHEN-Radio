// ====================================================================
//  QuchenRadio local desktop server
//  - 本地文件代理 / 本地节奏缓存 / 更新检查
...
function readUpdateConfig(pkg) {
  ...
  const owner = process.env.QUCHEN_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.QUCHEN_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.QUCHEN_UPDATE_MANIFEST
      || process.env.QUCHEN_UPDATE_MANIFEST_URL
      || process.env.QUCHEN_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.QUCHEN_UPDATE_MIRRORS || process.env.QUCHEN_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `QuchenRadio-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `QuchenRadio-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('QuchenRadio v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `QuchenRadio/${APP_VERSION}` },
    });
...
function localUpdateFallback(reason, opts) {
  ...
  return {
    ...
    release: {
      ...
      name: 'QuchenRadio v' + APP_VERSION,
      version: APP_VERSION,
      htmlUrl: '',
      downloadUrl: '',
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `QuchenRadio-${latestVersion}-Setup.exe`;
  ...
  return {
    ...
    release: {
      ...
      name: 'QuchenRadio v' + latestVersion,
      version: latestVersion,
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      signal: controller.signal,
      headers: {
        'User-Agent': `QuchenRadio/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('QuchenRadio v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `QuchenRadio-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `QuchenRadio-${version || APP_VERSION}.exe`;
}
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'quchen-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      ...
      productName: APP_PACKAGE.productName || 'QuchenRadio',
      version: APP_VERSION,
      update: {
