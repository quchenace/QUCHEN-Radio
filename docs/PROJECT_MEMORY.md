...
- 可运行程序：`E:\桌面\播放器软件\Quchen-Radio\Mineradio.exe`
...
- 关键参数/实现：本机已观察到 `%APPDATA%\Quchen-Radio\Preferences` 内 `partition.per_host_zoom_levels` 记录 `127.0.0.1: -1.0`；临时优先尝试 `Ctrl+0`，兜底清理 Preferences 中的 `per_host_zoom_levels`，不要删除整个 `%APPDATA%\Mineradio`。
...
- 关键参数/实现：真实代码/Git 仓库移动到 `E:\桌面\播放器软件\Quchen-Radio\resources\app`；可运行程序在 `E:\桌面\播放器软件\Quchen-Radio\Mineradio.exe`。
