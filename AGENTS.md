...
- 当前可运行程序：`E:\桌面\播放器软件\QUCHEN Radio\QUCHENRadio.exe`
...
前端主逻辑在 `public/index.html`。这个目录是正在运行的 `QUCHENRadio.exe` 使用的 app 目录，所以改完后重启外层 `E:\桌面\播放器软件\QUCHEN Radio\QUCHENRadio.exe` 就能及时检查效果。没有独立 npm test，改动后至少做：
